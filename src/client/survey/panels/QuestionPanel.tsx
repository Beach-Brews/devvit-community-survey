    ...
    return (
        <div className="flex flex-col h-full">
            <div className="p-8 flex-grow flex flex-col gap-4">
                ...
                <div className="text-xl">{question.description}</div>
                <div className="mt-8">
                    {renderQuestionInput()}
                </div>
            </div>
            <div className="mt-8 w-full flex flex-col justify-center items-center">
                <button disabled={disableNext} onClick={!disableNext ? onNext : undefined} className={`w-1/2 bg-blue-100 dark:bg-blue-900 disabled:bg-neutral-100 disabled:dark:bg-neutral-900 px-8 py-2 rounded-xl ${disableNext ? 'cursor-not-allowed' : ' cursor-pointer'}`}>
